...
public class SimpleKeyListener implements KeyListener {
	...
	public void keyPressed(KeyEvent arg0) {
		...
		if (controller.getDriver() instanceof ManualDriver) {
			if (controller.getState() == StateGUI.STATE_PLAY) {
				if (uikey == UserInput.Up) {
					((ManualDriver) controller.getDriver()).keyUp();
					return;
				} else if (uikey == UserInput.Down) {
					((ManualDriver) controller.getDriver()).keyDown();
					return;
				} else if (uikey == UserInput.Left) {
					((ManualDriver) controller.getDriver()).keyLeft();
					return;
				} else if (uikey == UserInput.Right) {
					((ManualDriver) controller.getDriver()).keyRight();
					return;
				}
			}
		}
